...
if __name__ == '__main__':
    ...
    src_path = os.path.abspath(os.path.join(package_path, '..'))
    
    if src_path not in sys.path:
        sys.path.insert(0, src_path)
    from fcfd_laser.utils import constants
else:
    from ..utils import constants

# Motors setup and control

class Motors:

    def __init__(self,
                 Motor_X = r"xi-com:///dev/ttyACM2",  # assigned the motor axes
        ...
        if logger is None:
            logger = logging.getLogger("fcfd.motors")
    ...
    def initialize_devices(self, step_to_um_conversion_coeff = 2.5):
        ...
        self.logger.info(f"Initializing Motor")
    ...
    def close_devices(self):
        ...
        self.logger.info("Disconnect device")
    ...
    def log_state(self):
        ...
        self.logger.info(f"Motors State - X: {posx.Position} um, "
    ...
    def a_scan(self, wait_time=100, step_in_um = 0, Num_of_steps = 0, verbose=False):
        ...
        while Num_of_steps != 0:
            if current_axis == 'X': # Note 0 is False
                ...
                if verbose: self.logger.info(f"Moving Motors X by {step_in_um} um")
                self.axis_x.command_wait_for_stop(wait_time)
            if current_axis == 'Y': # Note 0 is False
                self.axis_y.command_movr_calb(step_in_um)
                if verbose: self.logger.info(f"Moving Motors Y by {step_in_um} um")
                self.axis_y.command_wait_for_stop(wait_time)
            if current_axis == 'Z': # Note 0 is False
                self.axis_z.command_movr_calb(step_in_um)
                if verbose: self.logger.info(f"Moving Motors Z by {step_in_um} um")
                self.axis_z.command_wait_for_stop(wait_time)
...
if __name__ == "__main__":
    ...
    home_coordinates = [45400, 35000]                 # Approximate docking for FCFD 1.1
